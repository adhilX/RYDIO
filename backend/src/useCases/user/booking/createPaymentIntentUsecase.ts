...
export class CreatePaymentIntentUsecase implements IcreatePaymentIntentUsecase{
    constructor(private stripeService: IStripeService,private redisService: IredisService) {
  ...
  console.log('booking data from payment intent usecase ',bookingData)
    ...
    const isHeld =await await this.redisService.get(redisKey);
     
if(isHeld && isHeld !== bookingData.user_id)throw new Error('this vehicle is currently held by another user.')
      await this.redisService.set(redisKey,  600,bookingData.user_id); 
      return  await this.stripeService.createPaymentIntent(bookingData.total_amount)
    }
}